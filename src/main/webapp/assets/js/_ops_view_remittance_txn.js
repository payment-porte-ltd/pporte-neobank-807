...
function fnGetFiatTxn(){
    ...
    $('#get-txn-form input[name="rules"]').val('get_remittance_txn');
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#card_txn_header').html('Remittance Transactions')
            if(data.error=='false'){
                ...
                <tbody>`
                    for (i=0; i<txnList.length;i++){
                        htmlTable+= `<tr>
                                        ...
                                        <td>`+txnList[i].destinationAmount+`</td>
                                        <td>`+txnList[i].destinationAssetCode+`</td>
 										<td>`+txnList[i].status+`</td>
...
function fnGetFilteredTxn(){
   $( "#filter-txn-form" ).validate( {
    ...
    if($( "#filter-txn-form" ).valid()){
        ...
        $('#filter-txn-form input[name="rules"]').val('get_remittance_txn_btn_dates');
