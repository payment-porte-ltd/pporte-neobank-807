...
function fnGetSearchType(){
	...
	$('#searchbycard').append(serachHtml);
		...
		function fnViewMoreCustomers() {
				...
				$('#searchspecificcust-form input[name="qs"]').val('opstxn');
				$('#searchspecificcust-form input[name="rules"]').val('view_more_porte_txn_details');
...
function fnOpsViewSpecificCustomerTxn(publicKey, custname, relno){
	...
	$('input[name="qs"]').val('opstxn');
	$('input[name="rules"]').val('porte_transactions');
