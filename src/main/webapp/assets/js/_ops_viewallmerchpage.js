   i18next.init({
      ...
      resources: {
		...
		function fnEditMerchant(merchantcode){
			$('#pendingmerch-form').attr('action', fnGetOpsServletPath());
		    $('input[name="qs"]').val('opsmerch');
		    $('input[name="rules"]').val('opsshowspecificmerch');
			$('input[name="hdnmerchantcode"]').val(merchantcode);
			$("#pendingmerch-form").submit(); 
