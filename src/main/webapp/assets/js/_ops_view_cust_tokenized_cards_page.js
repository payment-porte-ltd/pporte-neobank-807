...
function fnGetSearchType(){
	...
	$('#searchbycard').append(serachHtml);
		...
		function fnViewMoreCustomers() {
				...
				$('#searchspecificcust-form input[name="rules"]').val('view_more_card_details');
		...
		function fnOpsViewSpecificCards(customerrelno,tokenid){
			console.log('customerrelno is ',customerrelno)
			$('#pendingcust-form').attr('action', fnGetOpsServletPath());
		    $('input[name="qs"]').val('opscust');
		    $('input[name="rules"]').val('opsshowallcards');
			$('input[name="hdncustomerrelno"]').val(customerrelno);
			$('input[name="hdntokenid"]').val(tokenid);
