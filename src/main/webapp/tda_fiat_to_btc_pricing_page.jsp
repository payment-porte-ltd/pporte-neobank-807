...
<%
ArrayList<AssetCoin> arrBTCPricing = null; ConcurrentHashMap<String, String> hashStatus = null; ArrayList<CryptoAssetCoins> arrAssets = null;
...
try{
	...
	if (request.getAttribute("btcpricing") !=null) arrBTCPricing = (ArrayList<AssetCoin>) request.getAttribute("btcpricing");
...
<!-- Title -->
<title>Fiat to BTC Pricing</title>
...
<body class="app sidebar-mini rtl">
								...
								<div class="col-md-12 col-lg-12">
									<div class="card" style="margin-top: -100px;">
										<div class="card-header">
											<div class="card-title">BTC Pricing against USD</div>
											<div class="ml-auto">
										...
										<div class="card-body">
											<div class="table-responsive">
												<table id="viewbtcxpricingtable"
													...
													<tbody>
														<%
																			if (arrBTCPricing != null) {
																				for (int i = 0; i < arrBTCPricing.size(); i++) {
																			%>
														<tr>
															<td><%=((AssetCoin) arrBTCPricing.get(i)).getAssetCode()%></td>
															<td>1 <%=((AssetCoin) arrBTCPricing.get(i)).getAssetCode()%>
																= $ <%=((AssetCoin) arrBTCPricing.get(i)).getSellingRate()%></td>
															<td><%=((AssetCoin) arrBTCPricing.get(i)).getOnMarkupRate()%></td>
															<td><%=((AssetCoin) arrBTCPricing.get(i)).getOffMarkupRate()%></td>
															<%if(((AssetCoin) arrBTCPricing.get(i)).getStatus().equals("A")){ %>
															<td style="color: #2962ff"><%=hashStatus.get( ((AssetCoin) arrBTCPricing.get(i)).getStatus())%></td>
															<%}else{ %>
															<td><%=hashStatus.get( ((AssetCoin) arrBTCPricing.get(i)).getStatus())%></td>
															<%} %>
															<td><%=((AssetCoin) arrBTCPricing.get(i)).getCreatedOn()%></td>
															<%if(((AssetCoin) arrBTCPricing.get(i)).getStatus().equals("A")){ %>
															...
															<td class="text-center align-middle">
																<div class="btn-group align-top">
																	<button class="btn btn-sm btn-primary badge"
																		...
																		onClick="javascript:fnEditAssetPrice('<%=((AssetCoin) arrBTCPricing.get(i)).getAssetCode()%>',
																							'<%=((AssetCoin) arrBTCPricing.get(i)).getSellingRate() %>','<%=((AssetCoin) arrBTCPricing.get(i)).getStatus()%>','<%=((AssetCoin) arrBTCPricing.get(i)).getCreatedOn()%>',
																							'<%=((AssetCoin) arrBTCPricing.get(i)).getSequenceNo()%>'
																							);return false;">
	...
	<script src="assets/js/_tda_fiat_to_btc_pricing_page.js"></script>
...
<%
	...
	if (arrBTCPricing!=null) arrBTCPricing=null; if(hashStatus!=null)hashStatus=null;
